...
// =====================
// Server-side message sync (catches messages that webhooks miss)
// =====================
const lastKnownTimestamps = {}; // instanceName -> { remoteJid -> lastTimestamp }
let syncInstances = []; // populated by /instance/fetchInstances calls

function startMessageSync() {
  setInterval(async () => {
    if (sseClients.size === 0) return; // No clients connected, skip
    for (const instName of syncInstances) {
      try {
        const res = await new Promise((resolve, reject) => {
          const url = new URL('/chat/findChats/' + instName, EVO_API_URL);
        ...
        if (!lastKnownTimestamps[instName]) lastKnownTimestamps[instName] = {};
        const known = lastKnownTimestamps[instName];

        res.forEach(chat => {
          const jid = chat.remoteJid;
          if (!jid || jid === 'status@broadcast' || jid === '0@s.whatsapp.net') return;
          const lm = chat.lastMessage;
          if (!lm || lm.key?.fromMe) return;
          const ts = typeof lm.messageTimestamp === 'string' ? parseInt(lm.messageTimestamp) : (lm.messageTimestamp || 0);
          if (ts <= 0) return;

          const prev = known[jid] || 0;
          if (ts > prev) {
            known[jid] = ts;
            if (prev > 0) {
              // New message detected — broadcast as synthetic messages.upsert event
              broadcastSSE('messages.upsert', {
...
// Auto-discover instances on startup and periodically
function discoverInstances() {
  const url = new URL('/instance/fetchInstances', EVO_API_URL);
  const client = url.protocol === 'https:' ? https : http;
  const req = client.request({
    hostname: url.hostname, port: url.port || undefined,
...
// Discover on startup (after 5s delay for API to be ready) and every 30s
setTimeout(discoverInstances, 5000);
setInterval(discoverInstances, 30000);
