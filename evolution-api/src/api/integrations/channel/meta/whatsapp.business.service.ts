          ...
          if (this.configService.get<S3>('S3').ENABLE) {
            try {
              ...
              if (!hasRealMedia) {
                ...
                messageRaw.keyId = messageRaw.key?.id;
                const createdMessage = await this.prismaRepository.message.upsert({
                  where: { instanceId_keyId: { instanceId: messageRaw.instanceId, keyId: messageRaw.keyId } },
                  create: messageRaw,
                  update: { status: messageRaw.status, message: messageRaw.message, messageTimestamp: messageRaw.messageTimestamp },
                });
