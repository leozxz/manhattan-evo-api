...
export class BaileysStartupService extends ChannelStartupService {
    ...
    'messages.upsert': async (
      ...
      try {
        for (const received of messages) {
          ...
          // Convert LID to phone JID early so DB and webhook use the same JID
          ...
          if (this.configService.get<Database>('DATABASE').SAVE_DATA.NEW_MESSAGE) {
            ...
            const msg = await this.prismaRepository.message.upsert({
              where: {
                instanceId_keyId: {
        ...
        const msg = await this.prismaRepository.message.upsert({
          where: {
            instanceId_keyId: {
              ...
              keyId: messageRaw.keyId || messageRaw.key?.id,
            },
          },
          create: messageRaw,
          update: {
  ...
  private prepareMessage(message: proto.IWebMessageInfo): any {
    ...
    const messageRaw = {
      ...
      keyId: message.key.id, // Extracted for unique constraint (instanceId + keyId)
      pushName:
        message.pushName ||
        (message.key.fromMe
          ? 'Você'
