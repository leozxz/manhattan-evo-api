...
function renderGraph(canvas, data, contactName) {
  ...
  // Relationship edges (cross-category correlations)
  ...
  // Auto-correlate entities that share keywords (e.g. patrimonio <-> corretora)
  const entityNodes = nodes.filter(n => n.type === 'entity');
  for (let i = 0; i < entityNodes.length; i++) {
    for (let j = i + 1; j < entityNodes.length; j++) {
      const a = entityNodes[i];
      const b = entityNodes[j];
      if (a.category === b.category) continue; // same category already linked via hub
      // Check if value of one mentions label of other or vice versa
  ...
  function drawEdge(e, highlight) {
    ...
    // Curved lines for relation/inferred edges
  ...
  function drawNode(n, highlight) {
    ...
    // Label inside for contact/category
    if (n.type !== 'entity') {
      ctx.fillStyle = '#fff';
  ...
  function draw() {
    ...
    // Find highlighted edges (connected to hovered node)
  ...
  const overlay = canvas.closest('.graph-modal-overlay');
